...
from form import CreateMemberForm,  resetpassword, CreateContactForm, getemail, loginpage,notepad,FoodForm
import requests
import openai
...
openai.api_key = "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"
...
def forgotpassword():
    ...
    return render_template('account_management/forgotpassword.html',email=getemails)
...
def reset(id):
    ...
    if request.method == 'POST' and reset.validate():
    ...
    if request.method == "POST" and form.validate():
        ...
        for member in member_list:
            if member.get_email() == email:
                ...
                flash('Email not register!', 'error')
    return render_template('account_management/login.html',login=form)
...
@app.route('/account_management/deleteacc/<int:id>/', methods=['GET','POST'])
def delete_acc(id):
    member_dict = {}
    db = shelve.open('storage.db', 'w')
    member_dict = db['member']
...
def fetch_and_preprocess_data(ingredient):
    api_key = 'c563ca0db36c4edba3a8d2b177'
    encoded_ingredient = quote(ingredient)
    recipe_id = str(random.randint(10,100))
...
def get_food(id):
    ...
    if request.method == 'POST' and form.validate():
        ...
        api_key = "c563ca0db36a7691978d2b177"
        recipe_id = '89'
        url = f'https://api.spoonacular.com/recipes/{recipe_id}/nutritionWidget.json?apiKey={api_key}'
        response = requests.get(url)
        if response.status_code == 200:

            nutrition_data = fetch_and_preprocess_data(foodname)
            calories = nutrition_data.get('calories', 'N/A')
            print(calories)

            jsonify(nutrition_data)
            print(jsonify(nutrition_data))

            return render_template('result.html',id=id,member=memberlogin,item=foodname,nutrition_data=nutrition_data,products=nutrition_data,calories=calories)
        else:
            print("not success")
...
def food_result(id):
    ...
    for member in member_list:
        if member.get_user_id() == id:
            memberlogin = member
            break
    api_key = 'c563c978d2b177'
    ingredient = 'banana'  # Assuming you have a form with an input field named 'ingredient'
    recipe_id = '1003464'
    ...
    if response.status_code == 200:
        ...
        return render_template('result.html', nutrition_data=nutrition_data,id=id,member=memberlogin)
    else:
        return 'Error fetching nutrition data'

    # return render_template('result.html',id=id,member=memberlogin)
